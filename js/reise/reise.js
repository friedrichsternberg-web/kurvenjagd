/* ============================================================================
   ...
   "Reise planen" selbst (5) und die Verkabelung (7).

   Was NICHT mehr hier steht: die Blaetter - neue Reise, umbenennen, Route
   waehlen, loeschen. Die liegen seit dem 11.09.2026 in blatt.js, weil
   diese Datei sonst ueber 1200 Zeilen gewachsen waere. Der Schnitt liegt
   an einer echten Fuge: Ein Blatt ist ein eigenes Fenster mit eigener
   Verkabelung, und mitfahrer.js und ausgaben.js haengen ihre eigenen
   Blaetter ohnehin schon von aussen daran.
...
function zeichneReisenListe() {
  ...
  behaelter.querySelectorAll('li[data-reise]').forEach(karte => {
    karte.addEventListener('click', ereignis => {
      /* Der Teilen-Knopf wird HIER abgefangen und nicht in teilen.js: Der
         Zuhoerer an der Karte sitzt naeher am Ziel und kaeme zuerst - die
         Reise ginge auf, bevor teilen.js ueberhaupt gefragt wird. */
      const teilen = ereignis.target.closest('[data-teile-reise]');
      if (teilen) {
        if (typeof teileReisePerLink === 'function') {
          teileReisePerLink(reiseNach(teilen.dataset.teileReise));
        }
        return;
      }
      oeffneReise(karte.dataset.reise);
    });
  });
    ...
    <li class="karte tour-karte reise-eintrag" data-reise="${escapeHtml(reise.id)}">
      ...
      <div class="widget-kopf">
        ...
        <span class="karte-werkzeuge">
          ${reise.serverId ? `<span class="reise-eintrag-marke" title="Gemeinsam geplant">${symbol('leute')}</span>` : ''}
          <button class="glas-rund karte-werkzeug" data-teile-reise="${escapeHtml(reise.id)}"
                  title="Reise als Link teilen" aria-label="Reise teilen">${symbol('teilen')}</button>
        </span>
      </div>
